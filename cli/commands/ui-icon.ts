...
import {logMissingArg} from '../../lib/helpers/message.js';
import {cleanDir, listDir} from '../../lib/helpers/file.js';
import {UI_OUTPUT_DIR_PATH} from '../../lib/helpers/project.js';
import {transpileAndOutputFiles} from '../../lib/helpers/typescript.js';
import {TS_CONFIG} from '../../lib/helpers/ui.js';
...
const ICON_TEMPLATE = `import {TiniIconComponent} from '@tinijs/ui/components/icon.js';
...
const PREVIEW_TS_TEMPLATE = `import {LitElement, html, css} from 'lit';
...
export default async function (
  ...
  if (!sources) return logMissingArg('sources');
  console.log('');
  ...
  const destDirPath = !commandOptions.output
    ? resolve(UI_OUTPUT_DIR_PATH, 'icons')
    : resolve(commandOptions.output);
  // clean
  await cleanDir(destDirPath);
  // build icons
  const icons = await buildIcons(spinner, sources, destDirPath, commandOptions);
  // result
...
async function buildIcons(
  ...
  const templateCode = ICON_TEMPLATE;
  const previewHtmlCode = PREVIEW_HTML_TEMPLATE;
  const previewTsCode = PREVIEW_TS_TEMPLATE;
  let filePaths = await readIconFilePaths(sources);
  ...
  for (let i = 0; i < filePaths.length; i++) {
    ...
    const componentNameClass = nameArr.map(name => capitalCase(name)).join('');
...
`;
  ...
  await transpileAndOutputFiles(
    (await listDir(destDirPath)).filter(path => path.endsWith('.ts')),
    TS_CONFIG,
    destDirPath,
...
async function readIconFilePaths(sources: string[]) {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (!~source.indexOf(':')) {
      const filePaths = (await listDir(resolve(source))).filter(imageFilter);
      ...
      for (let i = 0; i < suffixArr.length; i++) {
        ...
        const filePaths = (await listDir(resolve(dirRoot, dirName))).filter(
          imageFilter
        );
        allFilePaths.push(
          ...(!includesSuffix
