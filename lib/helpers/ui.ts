...
import {removeFiles, listDir} from './file.js';
import {transpileAndOutputFiles} from './typescript.js';

const {readdir, pathExists, outputFile, readFile} = fsExtra;
...
export function getCSSUtils() {
  const commonColorUtils = commonColorUtilGenerator();
  const commonGradientUtils = commonGradientUtilGenerator();
  const scalesUtils = factorUtilGenerator(SCALES);
  const sizeTextUtils = factorUtilGenerator(['--size-text']);
  const sizeSpaceUtils = factorUtilGenerator(['--size-space']);
  const sizeOutlineUtils = outlineUtilGenerator();
  const sizeBorderUtils = borderUtilGenerator();
  const sizeRadiusUtils = radiusUtilGenerator();
...
function factorUtilGenerator(keys: string[]) {
  return keys
    .map(key =>
      FACTORS.map(
        factor =>
...
function commonColorUtilGenerator() {
...
function commonGradientUtilGenerator() {
...
export async function devAndUseCopyGlobalFiles(
  destPath: string,
  isDev = false
) {}

export async function devAndUseBuildSkins(
  destPath: string,
  ...
  for (let i = 0; i < soulAndSkinsList.length; i++) {
    ...
    skins.forEach(skin => {
      const importPath = isDev
        ...
        : `../ui-${soul}/${STYLES_DIR}/skins/${skin}.css`;
      imports.push(`@import '${importPath}';`);
    });
  }
  const content = `${imports.join('\n')}\n${getCSSUtils()}`;
  await outputFile(resolve(destPath, 'skins.css'), content);
}

export async function devAndUseBuildBases(
  samplingPath: string,
  destPath: string,
  souls: string[],
  isDev = false
  ...
  for (let i = 0; i < baseNames.length; i++) {
    ...
    for (let j = 0; j < souls.length; j++) {
      ...
      const importPath = isDev
        ? `../${STYLES_DIR}/${soulName}/base/${baseName}`
        : `../ui-${soulName}/${STYLES_DIR}/base/${baseName}`;
  ...
  // remove bases.ts
  await removeFiles([basesOutPath]);
  // public path
  return `./${fileName.replace('.ts', '')}`;
}

export async function devAndUseBuildComponents(
  inputDir: string,
  destPath: string,
  souls: string[],
  isDev = false
  ...
  for (let i = 0; i < componentPaths.length; i++) {
      ...
      .reduce(
        (result, item) => {
          ...
          if (name) {
            souls.forEach(soul => {
              const importPath = isDev
                ? `../../${STYLES_DIR}/${soul}/base/${name}`
                : `../../ui-${soul}/${STYLES_DIR}/base/${name}`;
              result.imports.push(
    ...
    const soulContents = souls.reduce(
      (result, soul) => {
        const importPath = isDev
          ? `../../${STYLES_DIR}/${soul}/soul/${fileNameOnly}`
          : `../../ui-${soul}/${STYLES_DIR}/soul/${fileNameOnly}`;
        result.imports.push(
...
tagName: ${className}.defaultTagName,${
    ...
    // save file
    await outputFile(resolve(destPath, outputDir, filePath), code);
    await outputFile(
  ...
  if (componentPaths.length) {
    const outComponentsPaths = (
      await listDir(resolve(destPath, outputDir))
